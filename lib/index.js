...
function OrangeSMS(clientId, secret, options) {
	...
	if(!clientId || clientId === '' ||
		!secret || secret === '') {
		throw new Error('Orane client ID and secret are required.');
	}
	...
	function isValidCountryCode(countryCode) {
	    ...
	    return country && !countryCodeAlpha3Regex.test(country);
	}
			...
			if(authenticationData.token &&
	        	...
	        	var onAuthentication = function(err, im, body) {
		            ...
		            if(err) {
		                ...
		                authenticationData.expirationDate = new Date(Date.now() + authResponse.expires_in).getTime();
	...
	function sendSMS(receiverAddress, message, senderAddress, senderName) {
		...
		return getAuthToken().then(function(authData) {
			return new Promise(function(resolve, reject) {
		        ...
		        var smsEndpoint = config.uri.smsmessaging + config.uri.smsmessaging.replace('{senderAddress}', encodedSender);
		        apiCall(
		        	resolve,
		        	reject,
		        	authData,
